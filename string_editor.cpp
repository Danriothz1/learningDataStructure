...
void Editor::insert(int pos, const string &str)
{
    ...
    auto [text_begin, text_end] = _make_text(str);
    auto prev_ptr = texts[new_version];
    auto ptr = prev_ptr->next;
    int c_pos = 0;
    while (c_pos < pos && ptr)
    ...
    prev_ptr->next = text_begin;
    text_end->next = ptr;
    texts_size[new_version] += str.length();
...
void Editor::copy(int pos1, int len, int pos2)
{
    string cp;
    cp.reserve(len);
    auto ptr = texts[current_version]->next;
    int c_pos = 0;

    while (ptr && c_pos + ptr->block->str.size() <= pos1)
    {
        c_pos += ptr->block->str.size();
        ptr = ptr->next;
    }
    ...
    if (ptr != nullptr)
    {
        int start = pos1 - c_pos;
        int last = len;
        while (ptr && last > 0)
        {
            int to_take = min((int)ptr->block->str.length() - start, last);
            cp.append(ptr->block->str, start, to_take);
            last -= to_take;
            start = 0;
            ptr = ptr->next;
        }
    ...
    auto [text_begin, text_end] = _make_text(cp);
    auto prev_ptr = texts[new_version];
    auto insert_ptr = prev_ptr->next;
    c_pos = 0;
    while (c_pos < pos2 && insert_ptr)
    {
        c_pos += insert_ptr->block->str.size();
        prev_ptr = insert_ptr;
        insert_ptr = insert_ptr->next;
    }

    prev_ptr->next = text_begin;
    text_end->next = insert_ptr;
...
int Editor::query(int pos, int len, int ch)
{
    ...
    while (ptr != nullptr && c_pos + ptr->block->str.size() <= pos)
    {
        c_pos += ptr->block->str.size();
        ptr = ptr->next;
    }
    ...
    int len_in_first_block = min((int)ptr->block->str.length() - start, last);
    for (int i = 0; i < len_in_first_block; ++i)
    {
        if (ptr->block->str[start + i] == (char)('a' + ch))
        {
            count++;
        }
    }
    last -= len_in_first_block;
    ptr = ptr->next;

    while (ptr != nullptr && last > 0 && last >= ptr->block->str.size())
    {
        if (!ptr->block->str.empty())
        {
            count += ptr->block->counter[ch];
            last -= ptr->block->str.size();
        }
        ptr = ptr->next;
    }

    if (ptr != nullptr && last > 0)
    {
        int len_in_last_block = min((int)ptr->block->str.length(), last);
        for (int i = 0; i < len_in_last_block; ++i)
        {
            if (ptr->block->str[i] == (char)('a' + ch))
            {
                count++;
            }
        }
